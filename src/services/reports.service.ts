...
import { Report, CreateReportDto, UpdateReportDto } from '../types/report.types';
...
class ReportsService {
  ...
  async findByType(type: string): Promise<Report[]> {
    const response = await apiService.get<Report[]>(`${this.basePath}/type/${type}`);
    return response.data;
  }

  async findByProcess(processId: string): Promise<Report[]> {
    const response = await apiService.get<Report[]>(`${this.basePath}/process/${processId}`);
    return response.data;
  }

  async findByWorker(workerId: string): Promise<Report[]> {
    const response = await apiService.get<Report[]>(`${this.basePath}/worker/${workerId}`);
    return response.data;
  }
  ...
  async update(id: string, data: UpdateReportDto): Promise<Report> {
    const response = await apiService.patch<Report>(`${this.basePath}/${id}`, data);
    return response.data;
  }
