...
export const InviteWorkersModal = ({ processId, processName, onClose }: InviteWorkersModalProps) => {
  ...
  const inviteMutation = useMutation({
    mutationFn: async (workerIds: string[]) => {
      const promises = workerIds.map(workerId =>
        workersService.applyToProcess({
          workerId,
          processId,
          notes: invitationNotes || `Invitado por la empresa a postular al proceso: ${processName}`,
        })
      );
    ...
    onError: (error: any) => {
      ...
      setErrorMessage(error.response?.data?.message || 'Error al invitar trabajadores');
      setShowConfirmModal(false);
  ...
  // Filtrar workers que ya están en el proceso
  ...
  // Filtrar workers disponibles (que no estén ya en el proceso)
  const availableWorkers = workers.filter(worker => !existingWorkerIds.has(worker.id));
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xl font-semibold text-gray-900">Invitar Trabajadores</h3>
              <p className="text-sm text-gray-600 mt-1">Proceso: {processName}</p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <div className="mt-2 flex items-center justify-between text-sm text-gray-600">
            <span>{filteredWorkers.length} trabajadores disponibles</span>
            <span className="font-medium text-blue-600">{selectedWorkers.size} seleccionados</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {isLoading ? (
            <div className="text-center py-8 text-gray-500">Cargando trabajadores...</div>
                ...
                >
                  ...
                  <div className="ml-4 flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-1">
                      <p className="text-xs text-gray-500">{worker.email || 'Sin email'}</p>
                      <p className="text-xs text-gray-500">RUT: {worker.rut || 'N/A'}</p>
                      {worker.phone && (
                        <p className="text-xs text-gray-500">Tel: {worker.phone}</p>
                      )}
                    </div>
                  </div>
                  {worker.cvUrl && (
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-end gap-3">
          ...
          <button
            ...
            disabled={selectedWorkers.size === 0 || inviteMutation.isPending}
            className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {inviteMutation.isPending
              ? 'Invitando...'
              : `Invitar ${selectedWorkers.size > 0 ? `(${selectedWorkers.size})` : ''}`}
          </button>
        </div>
      </div>
      ...
      >
        <div className="space-y-4">
          ...
          <p className="text-center text-gray-700">
            ¿Confirmas que deseas invitar <strong>{selectedWorkers.size} trabajador(es)</strong> a este proceso?
          </p>
          <div className="flex gap-3 justify-end pt-4">
            ...
            >
              {inviteMutation.isPending ? 'Invitando...' : 'Aceptar'}
            </button>
          </div>
        </div>
      </Modal>
      ...
      >
        <div className="space-y-4">
          ...
          <p className="text-center text-gray-700">
            <strong>{selectedWorkers.size} trabajador(es)</strong> invitado(s) exitosamente al proceso
          </p>
