...
import { Test, CreateTestDto, TestType, QuestionType, TestTypeLabels, QuestionTypeLabels, CreateQuestionDto } from '../../types/test.types';
...
export default function TestModal({ test, onClose }: TestModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (test) {
        await updateMutation.mutateAsync({ id: test.id, data: formData });
      } else {
  ...
  const handleAddQuestion = () => {
    setFormData(prev => ({
      ...prev,
      questions: [...(prev.questions || []), { ...currentQuestion, order: (prev.questions?.length || 0) + 1 }],
    }));
    setCurrentQuestion({
      question: '',
      type: QuestionType.MULTIPLE_CHOICE,
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="flex gap-4">
              ...
              <label className="flex items-center">
                <input
                  ...
                  className="mr-2"
                />
                <span className="text-sm text-gray-700">Requiere Revisión Manual</span>
              </label>
            </div>
                  ...
                  <div key={index} className="flex items-start justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        {QuestionTypeLabels[q.type]} • {q.points} pts
                      </p>
                    </div>
                    <button
                      type="button"
                ...
                <div className="border rounded-lg p-4 bg-blue-50">
                  <div className="space-y-3">
                    ...
                    {(currentQuestion.type === QuestionType.MULTIPLE_CHOICE ||
                      currentQuestion.type === QuestionType.MULTIPLE_RESPONSE) && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          Opciones
                        </label>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            >
              {createMutation.isPending || updateMutation.isPending ? 'Guardando...' : 'Guardar Test'}
            </button>
          </div>
        </form>
      </div>
