...
export const Sidebar = () => {
    ...
    const isActive = (path: string) => {
        ...
        const isDashboardRoot = path === "/admin" || path === "/empresa" || path === "/evaluador" || path === "/trabajador" || path === "/invitado";

        if (isDashboardRoot) {
            return location.pathname === path;
        }

        // Para otras páginas, coincidir exactamente o con sub-rutas
        return (
    ...
    return (
        <>
            ...
            <div className={`fixed inset-y-0 left-0 w-72 glass-sidebar shadow-2xl z-40 border-r border-gray-200 transition-transform duration-300 flex flex-col ${
                ...
                {/* Logo */}
                <div className="flex-col p-4 lg:p-6 border-b border-gray-200 flex items-center bg-gradient-to-br from-primary-500/10 to-secondary-500/10 flex-shrink-0">
                    <div className="relative">
                        ...
                        <img
                            src={logo}
                            alt="Logo"
                            className="h-24 lg:h-36 w-auto relative z-10 drop-shadow-2xl"
                        />
                    </div>

                    {/* User Info Card */}
                    <div className="w-full mt-4 bg-white/70 rounded-xl p-3 shadow-md border border-gray-200">
                ...
                <nav className="flex-1 overflow-y-auto px-3 py-2">
                    {sections.map((section) => (
                        <div key={section.path}>
                            {/* Separator */}
                            {section.separator?.beforeThis && section.separator.label !== "Gesti\u00f3n" && (
                                <div className="my-4">
                                ...
                                className={`
                                    ...
                                    ${
                                        isActive(section.path)
                                            ? "bg-orange-500/20 text-orange-700 shadow-md"
                                            : "text-gray-800 hover:bg-orange-500/10 hover:text-orange-700"
                                    }
                                `}
                            >
                                <div className="flex items-center gap-3">
                                    <section.icon size={20} className={isActive(section.path) ? "text-orange-600" : "text-gray-600"} />
