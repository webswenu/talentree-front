...
export const GuestWorkerDetailPage = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-gray-500">Cargando información del trabajador...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p className="text-gray-900 mt-1">{worker.rut || '-'}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-gray-900 mt-1">
              {worker.birthDate ? new Date(worker.birthDate).toLocaleDateString('es-CL') : '-'}
            </p>
          </div>
          <div className="md:col-span-2">
            <label className="text-sm font-medium text-gray-500">Dirección</label>
            <p className="text-gray-900 mt-1">{worker.address || '-'}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-500">Ciudad</label>
            <p className="text-gray-900 mt-1">{worker.city || '-'}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-500">Región</label>
            <p className="text-gray-900 mt-1">{worker.region || '-'}</p>
          </div>
        </div>
      </div>
      ...
      {worker.applications && worker.applications.length > 0 && (
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold mb-6">Historial de Postulaciones</h2>
                  ...
                  <tr key={application.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {application.appliedAt
                        ? new Date(application.appliedAt).toLocaleDateString('es-CL')
                        : '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {application.updatedAt
                        ? new Date(application.updatedAt).toLocaleDateString('es-CL')
                        : '-'}
                    </td>
                  </tr>
                ))}
