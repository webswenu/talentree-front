...
import { UpdateSettingDto, GeneralSettings, NotificationSettings } from '../../types/settings.types';
...
export const SettingsPage = () => {
  ...
  // Initialize defaults on first load if empty
  useEffect(() => {
  ...
  useEffect(() => {
    if (notificationSettings) {
      setNotificationsForm({
        notifications_enabled: notificationSettings.notifications_enabled ?? true,
        email_notifications: notificationSettings.email_notifications ?? true,
        in_app_notifications: notificationSettings.in_app_notifications ?? true,
        notification_frequency: notificationSettings.notification_frequency || 'instant',
      });
    }
  ...
  const handleSaveGeneral = async (e: React.FormEvent) => {
    ...
    const settings: UpdateSettingDto[] = [
      { key: 'system_name', value: generalForm.system_name, isPublic: true },
      { key: 'contact_email', value: generalForm.contact_email, isPublic: true },
      { key: 'system_description', value: generalForm.system_description, isPublic: true },
  ...
  const handleSaveNotifications = async (e: React.FormEvent) => {
    ...
    const settings: UpdateSettingDto[] = [
      { key: 'notifications_enabled', value: notificationsForm.notifications_enabled },
      { key: 'email_notifications', value: notificationsForm.email_notifications },
      { key: 'in_app_notifications', value: notificationsForm.in_app_notifications },
      { key: 'notification_frequency', value: notificationsForm.notification_frequency },
    ];

    await upsertBatchMutation.mutateAsync(settings);
    setSuccessMessage('Configuración de notificaciones guardada correctamente');
    setTimeout(() => setSuccessMessage(''), 3000);
  };

  const isLoading = loadingGeneral || loadingNotifications || upsertBatchMutation.isPending;
          ...
          <form onSubmit={handleSaveNotifications} className="space-y-4">
            <div className="flex items-center">
              <input
                ...
                checked={notificationsForm.notifications_enabled}
            ...
            <div className="flex items-center">
              <input
                ...
                onChange={handleNotificationChange}
                className="h-4 w-4 text-blue-600 rounded"
                disabled={isLoading || !notificationsForm.notifications_enabled}
              />
              <label className="ml-2 text-sm text-gray-700">
