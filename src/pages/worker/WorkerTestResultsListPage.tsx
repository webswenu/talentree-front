...
export const WorkerTestResultsListPage = () => {
  ...
  const { data: applications, isLoading } = useWorkerProcesses(workerId || '');
  ...
  // Extraer todos los test responses completados
      ...
      ?.flatMap((app) =>
        app.testResponses
          ...
          .map((tr: TestResponse) => ({
            ...
            companyName: app.process?.company?.name || 'Empresa',
            completedAt: tr.completedAt ? new Date(tr.completedAt) : null,
            score: tr.score || 0,
            maxScore: tr.maxScore || 0,
            percentage: tr.maxScore ? Math.round(((tr.score || 0) / tr.maxScore) * 100) : 0,
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">Historial completo de tests realizados</p>
      </div>

      {/* Stats Cards */}
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white p-6 rounded-lg shadow">
          <p className="text-gray-500 text-sm mb-1">Tests Completados</p>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-3xl font-bold text-red-600">{stats.noAprobados}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {completedTests.length > 0 ? (
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {completedTests.map((test) => (
                <tr key={test.testResponseId} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">{test.testName}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{test.processName}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-500">{test.companyName}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-500">
                      {test.completedAt
                        ? test.completedAt.toLocaleDateString('es-CL', {
                            day: '2-digit',
                            month: '2-digit',
                            year: 'numeric',
                          })
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    >
                      {test.passed ? '✓ Aprobado' : '✗ No Aprobado'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button
                      onClick={() => navigate(`/trabajador/resultados/${test.testResponseId}`)}
          ...
          <div className="px-6 py-12 text-center">
            ...
            <p className="mt-4 text-gray-500 text-lg">No has completado ningún test todavía</p>
            <p className="mt-2 text-gray-400 text-sm">
              Completa tests de tus postulaciones activas para ver tus resultados aquí
            </p>
            <button
      ...
      {completedTests.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-start">
            ...
            <div>
              <h3 className="text-sm font-medium text-blue-800">Sobre tus resultados</h3>
              <div className="mt-2 text-sm text-blue-700">
                <p>
                  Los resultados mostrados corresponden a todos los tests que has completado en tus
                  postulaciones activas. Haz clic en "Ver Detalles" para revisar tus respuestas y
