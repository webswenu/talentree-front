...
export const WorkerProcessesPage = () => {
  ...
  const activeProcesses = processes?.filter((p) => p.status === ProcessStatus.ACTIVE) || [];
  const selectedProcess = processes?.find((p) => p.id === selectedProcessId);

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800">Procesos Disponibles</h1>
        <span className="text-sm text-gray-500">
          {activeProcesses.length} {activeProcesses.length === 1 ? 'proceso' : 'procesos'} disponibles
        </span>
      </div>
        ...
        {activeProcesses.map((process) => {
          ...
          return (
            <div key={process.id} className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
              ...
              <p className="text-gray-600 text-sm mb-4 line-clamp-2">{process.description || 'Sin descripción'}</p>
              ...
              <div className="space-y-2 mb-4">
                ...
                {process.location && (
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">{process.location}</span>
                  </div>
                )}
                {process.vacancies && (
                  <div className="flex items-center justify-between text-sm">
